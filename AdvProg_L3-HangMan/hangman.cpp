...
bool isCharInWord(const char ch, const string& word)
{
	...
	for(char i : word) if(i == ch)
    return true;
	return false;
}

char tolower(char c){
	if((c >= 'A') && (c <= 'Z')) c = char('a' + int(c - 'A'));
	return c;
}
...
string chooseWordFromList(const vector<string>& wordList, int index) 
{
    ...
    string answer = "";
	for(char i : wordList[index]) answer += tolower(wordList[index]);
    return answer;
}
...
string generateHiddenCharacters(string answerWord){
    ...
    string secretWord = "";
	for(int i = 0;i < int(answerWord.size());i ++) secretWord += "-";
    return secretWord;
}
...
void updateSecretWord(string& secretWord, const char ch, const string& word)
{
	...
	for(int i = 0;i < int(word.size());i ++) if(ch == word[i]) secretWord[i] = ch ;
}
